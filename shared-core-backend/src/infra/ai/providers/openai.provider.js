/**
 * OpenAI Chat Completions — 仅本文件使用网络与密钥组装；业务层禁止直调。
 */
...
async function chatComplete(opts) {
  ...
  const model = String(opts.model || config().aiModelDefault);
  const timeoutMs = Number(opts.timeoutMs) || config().aiTimeoutMs;
  ...
  try {
    ...
    try {
      ...
      throw new AppError("AI_EXECUTION_FAILED", `openai_invalid_json: ${text.slice(0, 200)}`, 502);
    }

    if (!res.ok) {
      const msg = json && json.error && json.error.message ? json.error.message : text.slice(0, 300);
      throw new AppError("AI_EXECUTION_FAILED", msg || `openai_http_${res.status}`, 502);
    }
